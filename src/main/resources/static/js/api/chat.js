...
// [1] 간단한 익명 아이지 만들어주기.
    ...
    // (Math.random() * 끝값 ) + 시작값 : 1부터 끝값 까지 사이의 난수
let randomId = Math.floor(Math.random()*1001) + 1 // 1~1000사이의 난수
...
clientSocket.onopen = (event) => {
    ...
    let msg = {'type' : 'alarm' , 'message': `${nickName}님이 입장했습니다.`}
...
clientSocket.onmessage = (event) => {
    ...
    // (2) 특정한 위치에 받은 메시지 출력하기 += 하는이유는 메시지를 누적 하기 위해
    const 채팅내용구역 = document.querySelector('.채팅내용구역');   
    
    if(message.type == 'alarm'){ // 만약에 메시지의 타입이 알람이면
        채팅내용구역.innerHTML+= `
                <div class="alarm"> <!-- 누군가 입장(알람 메시지)-->
                <span> ${message.message} </span>
                </div>`
    }else if(message.type == 'msg'){ // 만약에 메시지의 타입이 메시지 이면
        if(message.from == nickName){ // 메시지 보낸사람이 현재 nick 과 같으면( 내가보낸 메시지)
            채팅내용구역.innerHTML += `
            <div class="secontent"> <!-- 보낸 메시지 -->
                <span class="date"> ${message.date} </span>
                <span class="content"> ${message.message} </span>
            </div>`
        }else{ // (남이 보낸 메시지면)
            채팅내용구역.innerHTML += `
            <div class="receiveBox"> <!-- 받은 메시지 -->
...
// [3-2] 만약에 입력상자에서 엔터키 를 눌렀을때 메시지 전송
// onKeyup : 키보드 키를 누르고 떼었을때 이벤트
const enterKey = () => {
    // 만약에 엔터키를 눌렀을때
    if(window.event.keyCode == 13){ // C[대문자], 키보드 각 키들은 code번호가 존재.
        메시지전송();
    }
}
...
const 메시지전송 = () => {
    ...
    // (3) 메시지를 구성한다. type : 메시지 분류. message : 메시지 내용 , from : 보내는 사람, data : 현재날짜/시간
    let msg = {type:'msg' , message : 메시지 , from : nickName , date : new Date().toLocaleString()}

    // 2. 클라이언트 웹소켓 객체의 .send() 함수 이용 서버에게 메시지 전송
    clientSocket.send(JSON.stringify(msg));
