...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http ) throws Exception {
        ...
        http.authorizeHttpRequests( ( httpReq ) -> {
            ...
            httpReq
                    .requestMatchers( AntPathRequestMatcher.antMatcher("/board/write") ).authenticated() // 인증된(로그인된) 모든 회원
                    .requestMatchers( AntPathRequestMatcher.antMatcher("/chat") ).hasRole("USER") // 로그인된 모든 회원 접속
                    .requestMatchers( AntPathRequestMatcher.antMatcher("/api") ).hasRole("GENERAL") // 일반 회원만 접속
            ...
            // [3**] 403 에러 메시지 : 권한이 없을때 페이지 접근 차단. ( 권한없을경우 )
            // 403 에러 페이지 힌들러(매핑) 하기.
        ...
        http.formLogin( loginForm -> loginForm
                ...
                .loginProcessingUrl("/member/login") // 로그인을 처리할 요청 URL 정의 // POST 방식
