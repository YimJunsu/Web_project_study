...
public interface BoardRepository extends JpaRepository<BoardEntity, Integer> {
    ...
    @Query(value = "select * from where btitle like %:keyword%" , nativeQuery = true)
    List<BoardEntity> findByQuery2(String keyword);
        // 3. 특정한 카테고리 번호에 게시물제목에서 키워드를 포함하는 모든 게시물 조회
    ...
    List<BoardEntity> findByQuery4(String key, String keyword);
        // * 카테고리별(1. 전체조회 , 2. 제목btitle 검색 , 3. 내용bcontent 검색) + 페이징처리
